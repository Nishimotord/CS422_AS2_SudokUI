...
var makenote = false; // ADDED: toggle whether or not to make note
...
function redraw(givenstate, pos) {
  ...
  /*$(".timescore").css({
    visibility: state.seed == 1 && !victory ? "hidden" : "visible"
  });*/
...
$(document).on("click", "td.numberkey-cell", function(ev) {
  $(document).trigger("log", [
    "Number Palette Change",
    { type: ev.type, id: $(this).attr("id"), noteState: makenote }
  ]);
  var num = parseInt(
    $(this)
      .attr("id")
...
// ADDED: Keyboard shortcut for number palette (keycode 1-5 = 49-53, `=192)
$(document).on("keyup", function(ev) {
  ...
  if (num != null) {
    ...
    $(document).trigger("log", [
      "Number Palette Change",
      { type: ev.type, id: "nk" + num, noteState: makenote }
    ]);
  }
...
// REMOVED: the part of the function that resets to 'eraser'
...
$(document).on("mouseenter", "td.sudoku-cell", function(ev) {
  $(this)
    .find("div.sudoku-border")
    .toggleClass("focus", true);
...
$(document).on("mousedown", "td.sudoku-cell", function(ev) {
  ...
  redraw(state, pos);
  // MODIFIED: DON'T Clear the current number.
  // setcurnumber(0);
...
// Handles the next button. MODIFIED: for key presses

$(document).on("click", "#nextbutton", function(ev) {
  $(document).trigger("log", ["Next Game", { type: ev.type }]);
  flippage(1);
});
...
$(document).on("click", "#prevbutton", function(ev) {
  $(document).trigger("log", ["Prev Game", { type: ev.type }]);
  flippage(-1);
});
...
/////////////////////////////////////////////////////////////////////////////
// Clear/Check game handling MODIFIED: with keyup
/////////////////////////////////////////////////////////////////////////////
...
$(document).on("click", "#clearbutton", function(ev) {
  ...
  $(document).trigger("log", ["Clear Game", { type: ev.type }]);
  commitstate(cleared);
});
...
$(document).on("mousedown touchstart", "#checkbutton", function(ev) {
  ...
  $(document).trigger("log", ["Check Game Start", { type: ev.type }]);
  ev.stopPropagation();
});
...
$(document).on("mouseup mouseleave touchend", "#checkbutton", function(ev) {
  ...
  var state = currentstate();
  redraw(state);
  $(document).trigger("log", ["Check Game Stop", { type: ev.type }]);
});
...
// Defeat normal click handling for the "check" button.
...
// MODIFIED: 5 is eraser, 9 is note, handle control here
// Set the currently-selected number.  5 is the eraser,  ` is note
...
function updatetime() {
  if (runningtime && $(".timer").is(":visible")) {
    $(".timer").text(formatelapsed(new Date().getTime() - starttime));
    setTimeout(updatetime, 1001 - ((new Date().getTime() - starttime) % 1000));
  } else {
    runningtime = false;
  }
}
...
function shorttobase64(int18) {
  return base64chars[(int18 >> 6) & 63] + base64chars[int18 & 63];
}
...
function base64toshort(base64, index) {
  return (
    (base64chars.indexOf(base64.charAt(index)) << 6) +
...
function boardhtml() {
  ...
  text +=
    "<tr><td colspan=13 class=sudoku-border>" +
  ...
  for (var y = 0; y < Sudoku.N; y++) {
    ...
    for (var x = 0; x < Sudoku.N; x++) {
      var c = y * Sudoku.N + x;
      text +=
        "<td class=sudoku-cell id=sc" +
        c +
        ">" +
        "<div class=sudoku-border>" +
        "<div class=sudoku-number id=sn" +
        c +
        ">" +
        "&nbsp;</div></div>";
...
function numberkeyhtml() {
  ...
  result +=
    "<tr><td class=numberkey-cell id=nk9>" +
    '<div class="note nk9">' +
    "&#xf044;</div></td></tr>";
  for (var j = 1; j <= Sudoku.N; ++j) {
    result +=
      "<tr><td class=numberkey-cell id=nk" +
      j +
      ">" +
      '<div class="sudoku-answer nk' +
      j +
      '">' +
      handglyph(j) +
      "</div></td></tr>";
  ...
  result +=
    "<tr><td class=numberkey-cell id=nk0>" +
